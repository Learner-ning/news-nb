...
export function calculateHeatScore(item, ctx) {
  ...
  // 时间衰减项（freshness）。
  // 只有拿到真实发布时间（hasRealTime）才计算衰减；否则视为「新鲜度未知」，
  // 让该项取一个中性值，并把它让出的权重转给榜单名次与去重加成，
  // 避免「无时间」条目凭虚假满分新鲜度压过真正的新消息。
  let fresh = 0.5;
  ...
  // 时间衰减项权重：有真实时间用 0.30；无真实时间按 0.12 计（中性值已减半），
  // 省下的部分补给榜单名次，使热榜条目仍然按排名区分强弱。
