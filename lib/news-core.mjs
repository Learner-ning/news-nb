...
// 注意执行顺序：部分源把 HTML 做了 entity 转义（例如 IT之家/少数派/GitHub热榜/雪球
// 的 description 是 &lt;p&gt;…）。若「先删标签、后解码」，转义标签在删除阶段不可见、
// 解码之后又原样「复活」，最终把 HTML 标签当成正文输出。
// 正确顺序：解码 → 删 script/style → 删标签 → 再解码（处理残留实体）→ 空白归一化。
// 同时清理 CDATA 标记：部分源把 <![CDATA[ … ]]> 再做了 entity 转义，
// 解码后会以纯文本形式露出（雪球话题的正文尾部就出现过裸露的 ]]>）。
const CDATA_MARK = /<!\[CDATA\[|\]\]>/g;
...
export function stripHtml(html) {
  let s = decodeEntities(html)
    ...
    .replace(/<[^>]+>/g, " ");
  s = decodeEntities(s);
  // 兜底：多重转义的源仍可能残留标签与标记，再剥一次，保证输出永不含 HTML 标签。
  s = s.replace(CDATA_MARK, " ").replace(/<[^>]+>/g, " ");
  return s.replace(/\s+/g, " ").trim();
}

// 从原始内容（未清洗前）提取第一个有效图片地址；没有就返回 null，绝不伪造。
// 注意：部分源把 HTML 做了 entity 转义（&lt;img src="…"&gt;），必须先解码再匹配，
// 否则这些源的图片会全部漏抽。
...
export function parseFeedXml(xml) {
  ...
  if (isAtom) {
    ...
    for (const [, body] of entries) {
      ...
      const contentRaw = tag(body, "content") || tag(body, "summary") || "";
      // 先清洗、后截断：截断必须作用在纯文本上，否则会切出半个标签。
      const content = stripHtml(contentRaw);
      const updated = stripHtml(tag(body, "updated") || tag(body, "published"));
      if (title) out.push({ title, url, guid, content, image: extractImage(contentRaw), isoDate: updated || null });
    }
  } else {
    ...
    for (const [, body] of items) {
      ...
      // 关键：清洗必须在截断之前。contentRaw 可能仍是转义 HTML，先 stripHtml 得到纯文本，
      // 再截断，才能保证不会出现「半截标签」。
      const content = stripHtml(contentRaw);
      const summary = content.slice(0, 240);
      const isoDate = pub ? dateToIso(pub) : null;
      if (title) out.push({ title, url: url || guid || "", guid, summary, content, image: extractImage(contentRaw), isoDate });
    }
  }
  return out;
}
...
function buildRssItems(feed, entries) {
  ...
  return entries.slice(0, RSS_PER_FEED).map((e, i) => {
    ...
    return {
      ...
      content,
      // 图片来自 RSS 原始内容中的第一个 <img src>；没有就保持 null。
      image: e.image || null,
      publishedAt,
...
// 权重表：每一项都先归一化到 0~1 再加权，两个分支的权重之和都严格等于 1。
//   withTime（有真实发布时间）：时间衰减可放心占大头。
//   noTime  （无真实发布时间，主要是平台热榜）：时间项只保留中性值 + 低权重，
//   让出的 0.18 全部转给榜单名次，使热榜条目仍按排名区分强弱。
const HEAT_WEIGHTS = {
  withTime: { fresh: 0.30, quality: 0.12, rank: 0.16, dup: 0.16, cat: 0.08, kw: 0.18 },
  noTime:   { fresh: 0.12, quality: 0.12, rank: 0.34, dup: 0.16, cat: 0.08, kw: 0.18 }
};
// 源权重映射区间：源配置里的 weight 目前落在 0.9~1.15，直接代入会几乎无区分度，
// 这里把 0.8~1.2 线性展开成 0~1，让来源权重真正参与排序。
const FEED_W_MIN = 0.8;
const FEED_W_MAX = 1.2;
// 榜单名次归一化区间：每个平台取 15 条，第 1 名 → 1.0，第 16 名及以后 → 0。
const RANK_BEST = 1;
const RANK_WORST = 16;
// 多源重复归一化：1 个来源 → 0，4 个及以上来源 → 1。
const DUP_FULL = 3;
// 关键词命中数达到该值即取满。
const KW_FULL = 3;
...
export function calculateHeatScore(item, ctx) {
  ...
  const W = hasRealTime ? HEAT_WEIGHTS.withTime : HEAT_WEIGHTS.noTime;

  // ① 新鲜度 0~1：只有拿到真实发布时间才计算衰减；否则取中性值 0.5。
  let fresh = 0.5;
  if (hasRealTime) {
    let ageH = 24;
    try {
  ...
  // ② 来源权重 0~1：把 0.8~1.2 的源权重线性展开，避免"几乎无区分度"。
  const quality = clamp01(((Number(item.feedWeight) || 1) - FEED_W_MIN) / (FEED_W_MAX - FEED_W_MIN));

  // ③ 榜单名次 0~1：第 1 名取满，第 16 名及以后为 0；非榜单条目为 0。
  const rank01 = item.isHotBoard
    ? clamp01(1 - ((Number(item.rank) || RANK_WORST) - RANK_BEST) / (RANK_WORST - RANK_BEST))
    : 0;

  // ④ 多源重复 0~1：与 dedupe 的 dupCount 联动（1 个来源 0，4 个及以上取满）。
  const dup01 = clamp01(((Number(item.dupCount) || 1) - 1) / DUP_FULL);

  // ⑤ 分类热度 0~1：该分类条目数 / 最大分类条目数。
  let cat01 = 0;
  const catCount = ctx.catCount;
  if (catCount && typeof catCount.get === "function" && catCount.size) {
    let maxCat = 0;
    for (const n of catCount.values()) if (n > maxCat) maxCat = n;
    cat01 = clamp01((catCount.get(item.tag) || 0) / Math.max(1, maxCat));
  }

  // ⑥ 关键词 0~1：命中 KW_FULL 个即取满。
  let hits = 0;
  const title = String(item.title || "");
  for (const k of HEAT_KEYS) if (title.includes(k)) hits++;
  const kw01 = clamp01(hits / KW_FULL);
...
// ---------------- 列表 / 详情 输出裁剪 ----------------
// 列表接口（/api/news）只返回渲染树、列表、热榜真正需要的字段，不含正文 content。
// 正文体积约占整包 64%，而三个视图都不展示正文，只在详情页用得到。
// 详情接口（/api/news/:id）返回完整条目（含 content）。
export const LIST_FIELDS = [
  "id", "kind", "type", "category", "tag", "platform", "source",
  "title", "url", "summary", "image",
  "publishedAt", "time", "hasRealTime",
  "hotRank", "rank", "hotTag", "praise", "discuss",
  "dupCount", "heatScore"
];

// 列表摘要长度：悬停卡只显示前 90 字，120 字足够且显著压缩体积。
export const LIST_SUMMARY_LEN = 120;

/** 把完整条目裁剪成列表条目（纯函数；不改动缓存里的完整数据） */
export function toListItem(item) {
  const out = {};
  for (const k of LIST_FIELDS) out[k] = item[k] === undefined ? null : item[k];
  out.summary = String(item.summary || "").slice(0, LIST_SUMMARY_LEN);
  return out;
}
