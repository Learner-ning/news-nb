...
**影响面（真实浏览器 + 真实鼠标事件实测，2026-09-18 23:08）**：
**站内点击完全正常，坏的只是「刷新 / 直接输网址 / 分享链接」。**

```
【0】预热生产 isolate：反复请求 /api/news，第 5 次才拿到 215 条
【1】打开 /?mode=tree&env=day            → 树就绪=true，pathname=/
【2】真实鼠标点击 Solidot 节点             → / → /source/Solidot，源树已渲染=Solidot
【3】在该深链上按 F5 刷新                  → body="Not Found"，命中 Worker 末尾兜底分支
```

原因是 `app.js` 的站内跳转**全部走 `history.pushState`**（`app.js:389` / `394` / `419`），
不向服务器要这个路径；只有**重新请求该路径**时才会落到 Worker 的兜底分支。
⇒ 这是「**分享/收藏/刷新即坏**」，**不是「站内点不动」** —— 定性必须说到这个程度，不能笼统说「深链坏了」。

证据：`docs/验收截图/生产/生产-站内点击来源页-正常.png`、`生产-刷新深链-404.png`。
...
**为什么补这一步**：Stage 3.4/3.5 的 19 张截图全部打在 `review-server`（离线快照）或
`localhost:3000`（本地实时）上，**没有一张是打在 `newstree.dpdns.org` 上的** ——
「部署之后页面到底长什么样」此前从未被真正看过。

做法：把 `scripts/shots.mjs` 复制到系统临时目录后只改两处 —— 输出目录改到临时目录
（**避免覆盖已归档的截图**），`waitReady` 上限 60→150 次、详情 id 改为重试获取
（适配生产冷 isolate）；然后 `SHOT_BASE=https://newstree.dpdns.org node shots-prod.mjs`。

```
浏览器视觉验收（共 19 个场景）
  详情 id 就绪：1b6b02c9c581（第 5 次请求）
  ✓ day-tree-1366          1366×768   数据就绪 457KB
  ✓ day-list-1366          1366×768   数据就绪 262KB
  ✓ day-board-1366         1366×768   数据就绪 447KB
  ✓ night-tree-1366        1366×768   数据就绪 237KB
  ✓ night-list-1366        1366×768   数据就绪 219KB
  ✓ night-board-1366       1366×768   数据就绪 283KB
  ⚠ day-source-page-1366   1366×768   ⚠ 数据未就绪 5KB   ← 深链 404
  ⚠ night-source-page-1366 1366×768   ⚠ 数据未就绪 5KB   ← 深链 404
  ⚠ day-detail-1366        1366×768   ⚠ 数据未就绪 5KB   ← 深链 404
  ⚠ night-detail-1366      1366×768   ⚠ 数据未就绪 5KB   ← 深链 404
  ✓ day-hover-1366         1366×768   数据就绪 460KB  hover→ Solidot ✓高亮 ✓预览卡:Solidot
  ✓ day-1920               1920×1080  数据就绪 672KB
  ✓ night-1920             1920×1080  数据就绪 346KB
  ✓ day-mobile-390         390×844    数据就绪 173KB
  ✓ night-mobile-390       390×844    数据就绪 100KB
  ✓ dawn-variant-1366      1366×768   数据就绪 436KB
  ✓ ocean-variant-1366     1366×768   数据就绪 232KB
  ✓ day-photo-1366         1366×768   数据就绪 245KB
  ✓ night-photo-1366       1366×768   数据就绪 255KB
→ 15/19 数据就绪；4 个未就绪的正是 4 个深链场景
```

**结论**：生产域名上，首页树 / 列表 / 热榜（DAY、NIGHT 双环境）、1920 与移动端视口、
变体预设 dawn / ocean、自定义背景图 + 压暗层、以及 hover 预览卡（`✓高亮 ✓预览卡:Solidot`）
**全部渲染正常**，页面头部实测显示 `16 SOURCES / 225 STORIES`。
唯一不正常的仍然是深链（§4.1）。

归档：`docs/验收截图/生产/`（9 张）。**这 9 张按「图片不上传 GitHub」的约定只存在本地**，
远端不含 —— 与 §5、§8 的说明一致。
...
| **`/detail/:id`、`/source/:key` 生产 404** | ❌ **仍未修复**（§4.1 复验 + 影响面实测）。**站内点击正常，刷新 / 分享 / 收藏即坏**。成因未定论，需按 `DEPLOY.md` 在 Cloudflare 面板定位；本次推送内容不含部署配置改动，故未擅自改动 |
...
| PNG 未上传（25 + 本次 9 = 34 个） | 按用户指示「图片不用上传到 GitHub」。远端 `main` 不含这些文件（§5、§6.3），**未用占位文件冒充** |
